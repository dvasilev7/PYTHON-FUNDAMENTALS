...
def create_task(name, description, priority, is_completed):
    answer = messagebox.askquestion("Confirm Create", "Are you sure?")
    if answer == "no":
        return
    try:
        with open("dp.txt", "r") as file:
...
def render_create_view():
    ...
    Button(tk, text="Create task", bg="green", fg="white",
           command=lambda: create_task(task_name.get(), task_description.get("1.0", END), task_priority.get(),
...
if __name__ == "__main__":
    tk = Tk()
    tk.geometry("700x500")
    tk.title("To Do list application by DemOna")
    render_main_view()
    tk.mainloop()
